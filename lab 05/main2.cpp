...
int ObliczB(int ZadaneDane[][RPQ], int PiS[], int IleZadan){
    ...
    int TymczasowyCMax = 0, CmaxDoZadania;
    for (int i=0; i < IleZadan; i++) {	
        CmaxDoZadania = Cmax(ZadaneDane, i, PiS);        // NIE R TYLKO S_j
		if (CmaxDoZadania == TymczasowyCMax) {
			b = i;
			TymczasowyCMax = CmaxDoZadania;
		}
	}
    return b;
}
...
int ObliczA(int ZadaneDane[][RPQ], int PiS[], int IleZadan, int ZadB){
    int a;  // indeks zadania a
    int GlownyCmax = Cmax(ZadaneDane, IleZadan, PiS);
    int SumaSciezkiKryt, SumaP;

    for (int i=0; i < IleZadan; i++) {
        SumaP = 0;
        for (int s = i; s < ZadB; s++) {
...
int ObliczC(int ZadaneDane[][RPQ], int PiS[], int IleZadan, int ZadB, int ZadA){
    int c;  // indeks zadania c


    for (int i=ZadA; i < ZadB; i++) {	
		if (ZadaneDane[PiS[i]][2] < ZadaneDane[PiS[ZadB]][2]) {
            c = i;
		}
	}
    return c;
